...
// Read and modify the SVG to use the white color
const svgContent = fs.readFileSync(sourceSvg, 'utf8');
const whiteColorSvg = svgContent.replace(/currentColor/g, '#FFFFFF');
const tempSvgPath = path.join(__dirname, 'temp-icon.svg');
fs.writeFileSync(tempSvgPath, whiteColorSvg);

// Function to generate PNG favicon with plum background
async function generatePng(size, filename) {
  await sharp({
    create: {
      ...
      height: size,
      channels: 4,
      background: { r: 81, g: 98, b: 255, alpha: 1 }
    }
  })
